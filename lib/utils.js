...
module.exports = {
    ...
    getFileList: function(fileglob, buildDir, pkgJsonOverride) {
        var self = this,
            jsonfile, destFiles = [],
            srcFiles = [],
            package_path,
            matches = Glob(fileglob),
            tmpPkgPath = buildDir + '/temp/';
        ...
        return new Promise(function(resolve, reject) {
            ...
            matches.forEach(function(file) {
                ...
                if (internalFileName === path.normalize('build/dist/package.json')) {
                    ...
                    if (pkgJsonOverride) {
                        ...
                        var tempPath = path.normalize(tmpPkgPath + 'package.json');
                        var packageInfo = fs.readFileSync(jsonfile);
                        ...
                        // override the options in the package json.
                        for (var i in pkgJsonOverride) {
                            var pieces = i.split('.', 2);
                            if (pieces.length == 2) {
                                packageInfo[pieces[0]][pieces[1]] = pkgJsonOverride[i];
                            } else {
                                packageInfo[i] = pkgJsonOverride[i];
                            }
                        }

                        // create the temp folder if it doesn't exist.
                        if (!fs.existsSync(tmpPkgPath)) {
                            fs.mkdirSync(tmpPkgPath);
                        }

                        // write the temp package json file.
                        fs.writeFileSync(tempPath, JSON.stringify(packageInfo, null, 2));
            ...
            srcFiles.forEach(function(file) {
                ...
                if (pkgJsonOverride && file === path.normalize(tmpPkgPath + 'package.json')) {
                    dest = file.replace(path.normalize(tmpPkgPath), '');
                }
