...
function NwBuilder(options) {
    ...
    var defaults = {
        ...
        macPlist: false,
        winIco: null,
        packageJson: null // Overrides to the node webkit app package.json file.
    };
...
NwBuilder.prototype.checkFiles = function () {
    ...
    return Utils.getFileList(this.options.files, this.options.buildDir, this.options.packageJson).then(function (data) {
...
NwBuilder.prototype.cleanUp = function() {
    var self    = this,
        done    = Promise.defer(),
        tmpPath = path.normalize(path.join(self.options.buildDir, '/temp/'));

    if (self.options.packageJson && fs.existsSync(tmpPath)) {
